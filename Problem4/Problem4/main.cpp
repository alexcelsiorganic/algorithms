...
void MakeTree(BinaryTree &new_tree, Node *& old_root, int &counter, int height) {
    if (old_root != nullptr) {
        if (old_root->left != nullptr && old_root->right != nullptr) {
            if (old_root->height == height && old_root->left->heirs > old_root->right->heirs) {
                new_tree.insert(old_root->key);
                counter += 1;
            } else {
                MakeTree(new_tree, old_root->right, counter, height);
                MakeTree(new_tree, old_root->left, counter, height);
            }
        }
        else if (old_root->left != nullptr && old_root->right == nullptr) {
            if (old_root->height == height) {
                new_tree.insert(old_root->key);
                counter += 1;
            }
            else {
...
int main() {
    ...
    BinaryTree new_tree;
    if (need_height != 0) {
        MakeTree(new_tree, tree.root, amount_of_nodes, need_height);
        ...
        tree.root = tree.remove_node(tree.root->get_key(), tree.root);
    }
